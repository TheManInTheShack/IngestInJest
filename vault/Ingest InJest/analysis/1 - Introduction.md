...
Another level or so up we can say that we have combinations of words, which we typically call 'phrases', further classified into 'sentences'.  These are even more constrained in terms of the valid word combinations, so the infinity we started with is a LOT smaller by now.  
